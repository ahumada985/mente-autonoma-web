// EXTRACTOR DIRECTO DE DATOS REALES QUE SÍ FUNCIONA
...
async function processMatchesData(dataRows, headers, leagueInfo) {
  ...
  for (let i = 0; i < sampleRows.length; i++) {
    ...
    try {
      ...
      // Mostrar progreso cada 5 partidos
      if (matches % 5 === 0) {
...
async function processTeamsData(dataRows, headers, leagueInfo) {
  ...
  for (let i = 0; i < sampleRows.length; i++) {
    ...
    try {
      ...
      // Mostrar progreso cada 2 equipos
      if (teams % 2 === 0) {
...
async function processEventsData(dataRows, headers, leagueInfo) {
  ...
  for (let i = 0; i < sampleRows.length; i++) {
    ...
    try {
      ...
      // Mostrar progreso cada 5 eventos
      if (events % 5 === 0) {
...
async function main() {
  ...
  console.log('🎯 EXTRAYENDO FECHAS, EQUIPOS Y EVENTOS REALES');
