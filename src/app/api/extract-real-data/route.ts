// API Endpoint para Extraer Datos Reales de CSVs
...
export async function POST(request: NextRequest) {
  try {
    console.log('🔥 Iniciando extracción de datos reales de CSVs...');
    ...
    for (const filePath of csvFiles) {
      try {
        ...
        const fileResult = await extractRealDataFromFile(filePath);
    ...
    const response = {
      success: true,
      message: 'Extracción de datos reales completada exitosamente',
      data: {
        ...results,
        summary: {
          ...results.summary,
          ...
          nextSteps: [
            ...
            'Integrar fuentes adicionales (SofaScore, etc.)',
            'Implementar sistema de predicciones',
            'Crear análisis de oportunidades'
          ]
        }
      }
    };
    
    console.log('✅ Extracción de datos reales completada!');
    console.log(`📊 Resumen:`);
    ...
    console.error('Error en extracción de datos reales:', error);
    
    return NextResponse.json({
      success: false,
      message: 'Error en extracción de datos reales',
...
// Función para extraer datos reales de un archivo CSV
async function extractRealDataFromFile(filePath: string) {
  try {
    ...
    if (format === 'matches') {
      ...
      const result = await processEventsData(dataRows, headers, leagueInfo);
...
async function processMatchesData(dataRows: string[], headers: string[], leagueInfo: any) {
  let matches = 0;
  ...
  // Procesar solo las primeras 100 filas para evitar timeout
  ...
  for (const row of sampleRows) {
    ...
    try {
      ...
      console.warn(`Error procesando partido:`, error);
    }
  }
...
async function processTeamsData(dataRows: string[], headers: string[], leagueInfo: any) {
  let teams = 0;
  ...
  for (const row of sampleRows) {
    ...
    try {
      ...
      console.warn(`Error procesando equipo:`, error);
    }
  }
...
async function processEventsData(dataRows: string[], headers: string[], leagueInfo: any) {
  let events = 0;
  ...
  for (const row of sampleRows) {
    ...
    try {
      ...
      console.warn(`Error procesando evento:`, error);
    }
  }
