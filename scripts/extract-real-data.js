// Extractor de Datos Reales - "Gran Henki Dama de Goku"
// Script para extraer y guardar datos reales de fechas, equipos y eventos
...
const OUTPUT_FILE = 'real-data-extraction-report.json';

// Inicializar Prisma
const prisma = new PrismaClient();

// Función para extraer datos reales de un archivo CSV
async function extractRealDataFromFile(filePath) {
  try {
    console.log(`🔄 Extrayendo datos reales de: ${filePath}`);
...
async function processMatchesData(dataRows, headers, leagueInfo) {
  ...
  for (const row of dataRows) {
    ...
    try {
      ...
      console.warn(`Error procesando partido:`, error.message);
    }
  }
...
async function processTeamsData(dataRows, headers, leagueInfo) {
  ...
  for (const row of dataRows) {
    ...
    try {
      ...
      console.warn(`Error procesando equipo:`, error.message);
    }
  }
...
async function processEventsData(dataRows, headers, leagueInfo) {
  let events = 0;
  ...
  for (const row of dataRows) {
    ...
    try {
      ...
      console.warn(`Error procesando evento:`, error.message);
    }
  }
...
async function main() {
  ...
  console.log('📊 Extrayendo fechas, equipos y eventos de CSVs históricos');
  ...
  console.log('\n✅ Extracción de datos reales completada!');
  console.log(`📊 Resumen:`);
...
module.exports = { main, extractRealDataFromFile };
