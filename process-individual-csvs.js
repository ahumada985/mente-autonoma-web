...
function detectColumns(headers) {
  ...
  return {
    ...
    homeShots: headers.findIndex(h => 
      h.toLowerCase().includes('home_shots') || 
      h.toLowerCase().includes('hshots') ||
      h.toLowerCase().includes('shots1')
    ),
    awayShots: headers.findIndex(h => 
      h.toLowerCase().includes('away_shots') || 
      h.toLowerCase().includes('ashots') ||
      h.toLowerCase().includes('shots2')
    ),
    
    // Columnas de tarjetas
    homeCards: headers.findIndex(h => 
      h.toLowerCase().includes('home_cards') || 
      h.toLowerCase().includes('hcards') ||
      h.toLowerCase().includes('cards1')
    ),
    awayCards: headers.findIndex(h => 
      h.toLowerCase().includes('away_cards') || 
      h.toLowerCase().includes('acards') ||
      h.toLowerCase().includes('cards2')
    ),
    
    // Columnas de córners
    homeCorners: headers.findIndex(h => 
      h.toLowerCase().includes('home_corners') || 
      h.toLowerCase().includes('hcorners') ||
      h.toLowerCase().includes('corners1')
    ),
    awayCorners: headers.findIndex(h => 
      h.toLowerCase().includes('away_corners') || 
      h.toLowerCase().includes('acorners') ||
      h.toLowerCase().includes('corners2')
    )
  };
}

// Procesar un archivo CSV individual
async function processIndividualCSV(filePath) {
  try {
    console.log(`\n🔄 PROCESANDO: ${filePath}`);
    ...
    if (lines.length < 2) {
      ...
      return { matches: 0, teams: 0, events: 0 };
    }
    ...
    // Procesar solo las primeras 10 filas para evitar timeout
    ...
    console.log(`✅ ${filePath}: ${matches} partidos, ${teams} equipos procesados`);
    return {
      matches,
      teams,
      events,
...
// Función principal
async function main() {
  console.log('🔥 INICIANDO PROCESAMIENTO INDIVIDUAL DE CSVs...');
  console.log('📊 "Gran Henki Dama de Goku" - Sistema de análisis de apuestas deportivas');
  console.log('🎯 PROCESANDO CADA ARCHIVO INDIVIDUALMENTE Y GUARDANDO DATOS REALES');
  ...
  for (let i = 0; i < csvFiles.length; i++) {
    ...
    try {
      const fileResult = await processIndividualCSV(filePath);
  ...
  // Guardar reporte
  fs.writeFileSync('individual-csv-processing-report.json', JSON.stringify(results, null, 2));
  
  const processingTime = Date.now() - startTime;
  
  console.log('\n🎉 ¡PROCESAMIENTO INDIVIDUAL COMPLETADO!');
  ...
  console.log(`   📄 Reporte guardado en: individual-csv-processing-report.json`);
...
module.exports = { main, processIndividualCSV };
